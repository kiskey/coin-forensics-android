...
object DieReferenceAnalyzer {
    fun assess(
        ...
        val interpretation = when {
            ...
            genuineRegionConsensus.isEmpty() -> "The available physical/specification checks can be summarized, but no die-level image comparison against a documented genuine control is available yet. Add obverse/reverse genuine controls before interpreting the score as visual consistency."
            checks.any { it.status == DiagnosticStatus.INCONSISTENT } -> "One or more measured areas are materially inconsistent with the attached genuine consensus or physical specification. Recheck orientation, perspective, variety, wear and source provenance before treating this as counterfeit evidence."
